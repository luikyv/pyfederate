...
class TestTokenInfo:
    ...
    def test_to_jwt_payload(
        ...
        assert token_info.to_jwt_payload() == expected_payload,\
...
class TestJWTTokenModel:
    ...
    def test_generate_token(
        ...
        jwt_token: str = jwt_token_model.generate_token(
...
class TestClient:
    ...
    def test_are_scopes_allowed(
        ...
        assert secret_authenticated_client.are_scopes_allowed(conftest.SCOPES),\
            "The scopes should be allowed"
        assert not secret_authenticated_client.are_scopes_allowed(["invalid_scope"]),\
    ...
    def test_owns_redirect_uri(
        ...
        assert secret_authenticated_client.owns_redirect_uri(conftest.REDIRECT_URI),\
            "The client owns the redirect uri"
        assert not secret_authenticated_client.owns_redirect_uri("invalid_redirect_uri"),\
    ...
    def test_are_response_types_allowed(
        ...
        secret_authenticated_client.response_types = [
            constants.ResponseType.CODE]

        assert secret_authenticated_client.are_response_types_allowed([constants.ResponseType.CODE]),\
            "The response types are allowed"
        assert not secret_authenticated_client.are_response_types_allowed([constants.ResponseType.ID_TOKEN]),\
    ...
    def test_is_grant_type_allowed(
        ...
        assert secret_authenticated_client.is_grant_type_allowed(grant_type=constants.GrantType.AUTHORIZATION_CODE),\
            "The grant type should be allowed"
        assert not secret_authenticated_client.is_grant_type_allowed(grant_type=constants.GrantType.CLIENT_CREDENTIALS),\
            "The grant type should not be allowed"


class TestClientIn:

    def test_only_authz_code_has_response_types(self, client_in: schemas.ClientIn) -> None:
    ...
    def test_client_without_authn_method_must_require_pkce(self, client_in: schemas.ClientIn) -> None:
        with pytest.raises(ValueError):
            schemas.ClientIn(
                **{
                    **dict(client_in),
