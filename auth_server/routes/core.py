...
async def set_telemetry_ids(request: Request, call_next) -> Response:
    ...
    x_correlation_id: str | None = request.headers.get(constants.HTTPHeaders.X_CORRELATION_ID.value)
    if(x_correlation_id): telemetry.correlation_id.set(x_correlation_id)
